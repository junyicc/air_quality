            ...
            addMapOverlay: function () {
                ...
                PageMethods.GetLocation(function (e) {
                    ...
                    for (i = 0; i < stationsJson.items.length; i++) {
                        ...
                        //符号颜色，点符号
                        var PointSymbol = new PictureMarkerSymbol("widgets/MyWidgets/StationLoader/images/station.png", 20, 25);
                    ...
                    graphicsLayer.on("click", function (e) {
                        ...
                        if (esriConfig.defaults.warningGraphicsLayerId != null) {
                            var removeLyr = Map.getLayer(esriConfig.defaults.warningGraphicsLayerId);
                            Map.removeLayer(removeLyr);
                        }
                        Map.removeLayer(esriConfig.defaults.LocationGraphicsLayer);
                        var warningAttr = {
                            "Name": "warning"
                        };
                        var warningGraphicsLayer = new GraphicsLayer();
                        var warningSymbol = new PictureMarkerSymbol("widgets/MyWidgets/StationLoader/images/warning.gif", 50, 50)
                        warningSymbol.yoffset = -10;
                        var warningGra = new Graphic(geo, warningSymbol, warningAttr);
                        warningGraphicsLayer.add(warningGra);
                        Map.addLayer(warningGraphicsLayer);
                        esriConfig.defaults.warningGraphicsLayerId = Map.graphicsLayerIds[Map.graphicsLayerIds.length - 1];
                        Map.addLayer(esriConfig.defaults.LocationGraphicsLayer);
                        ...
                        PageMethods.GetNewestData(Name, function (e) {
                            ...
                            Map.infoWindow.on("hide", function (e) {
                                if (esriConfig.defaults.warningGraphicsLayerId != null) {
                                    var removeLyr = Map.getLayer(esriConfig.defaults.warningGraphicsLayerId);
                                    Map.removeLayer(removeLyr);
                                    esriConfig.defaults.warningGraphicsLayerId = null;
                                }
                            });
